...
async def lab_run(request: Request):
    ...
    if not 16 <= max_tokens <= 32768:
        return JSONResponse({"error": "max_tokens must be 16-32768"}, status_code=400)
    if _lab_busy():
...
<div class=tab id=lab>
 ...
 <div class=actions>
  ...
  <label>Max tokens <input id=labMaxTok type=number value=32768 min=16 max=32768 style=width:90px></label>

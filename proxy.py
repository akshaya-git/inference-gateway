...
function applyLabAvailability(){if(!labOptions)return;const model=$('labModel'),framework=$('labFramework');if(!model||!framework)return;const mSpec=labOptions.models.find(m=>m.key===model.value);for(const opt of framework.options){const ok=!mSpec||!!mSpec.frameworks[opt.value];opt.disabled=!ok}const fSpec=labOptions.frameworks.find(f=>f.key===framework.value);for(const opt of model.options){const spec=labOptions.models.find(m=>m.key===opt.value);const ok=!fSpec||!!(spec&&spec.frameworks[opt.value]);opt.disabled=!ok}if(framework.options[framework.selectedIndex]&&framework.options[framework.selectedIndex].disabled){const first=[...framework.options].find(o=>!o.disabled);if(first)framework.value=first.value}if(model.options[model.selectedIndex]&&model.options[model.selectedIndex].disabled){const first=[...model.options].find(o=>!o.disabled);if(first)model.value=first.value}}
